...
QuadData createQuad(const glm::vec3& q, const glm::vec3& u, const glm::vec3& v) {
    ...
    data.v = v;
    auto n = cross(u, v);
    data.normal = normalize(n);
    data.D = dot(data.normal, data.Q);
    data.w = n / dot(n, n);
    return data;
}

void addSphere(SceneData& scene, const SphereData& s) {
    scene.list.spheres.push_back(s);
}

void addQuad(SceneData& scene, const QuadData& s) {
    scene.list.quads.push_back(s);
}

void addCube(SceneData& scene, glm::vec3 position, float width, MaterialData material) {

}

void addRectangle(SceneData& scene, glm::vec3 position, float width, float height, float length) {
