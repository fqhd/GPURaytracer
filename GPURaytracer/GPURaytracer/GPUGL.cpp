...
void GPUGL::UploadSceneData(const SceneData& data)
{
	...
	std::cout << data.list.spheres.size() << std::endl;
	std::cout << data.list.quads.size() << std::endl;
	m_Shader->loadUniform("numSpheres", (int)data.list.spheres.size());
	m_Shader->loadUniform("numQuads", (int)data.list.quads.size());
	...
	for (int i = 0; i < data.list.spheres.size(); i++) {
		...
		m_Shader->loadUniform(sphere + ".material.brightness", data.list.spheres[i].material.brightness);
	...
	for (int i = 0; i < data.list.quads.size(); i++) {
		...
		m_Shader->loadUniform(quad + ".material.brightness", data.list.quads[i].material.brightness);
